...
const Slider: React.FC<SliderProps> = ({ label, min = 0, max = 100, step = 1, value, onChange, className = "", unit = "", infoText }) => {
    ...
    return (
        <div className={`w-full ${className}`}>
            {label && (
                <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center gap-2">
                        <label className="text-sm font-medium text-purple-600 dark:text-purple-400">{label}</label>
                        {infoText && (
                            <Tooltip content={infoText}>
                                <Info className="size-4 text-slate-400 hover:text-purple-500 cursor-help transition-colors" />
                            </Tooltip>
                        )}
                    </div>
            ...
            <div className="relative w-full h-2 text-purple-600 dark:text-purple-400 rounded-full cursor-pointer">
                {/* Track fill */}
